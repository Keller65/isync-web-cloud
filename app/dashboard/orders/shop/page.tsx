...
function ProductCard({ product }: { product: Product }) {
  ...
  const handleAddToCart = useCallback(() => {
    ...
    if (!product || quantity <= 0 || !isPriceValid || finalPriceForCart <= 0) {
      alert('Por favor, asegúrate de que la cantidad sea mayor a 0 y el precio sea válido.')
      return
    }
    ...
    if (totalQuantity > maxStock) {
      alert(
        `Stock insuficiente. Solo hay ${maxStock} unidades disponibles en stock. ${itemInCart ? `Ya tienes ${currentCartQuantity} en el carrito.` : ''}`
      )
      return
    }
    ...
    if (itemInCart) {
      if (confirm(`${product.itemName} ya está en tu carrito. ¿Actualizar cantidad y precio?`)) {
